---
title: "eCOTS Demo 2022 - solutions"
...
You may notice some code below has already been pre-populated for you. In these cases, there is a flag set as \texttt{eval = FALSE}. Make sure to remove this flag prior to running the relevant code chunk to avoid any errors when rendering the pdf.
...
It appears that the marginal distribution for interest rates is largely unimodal (possibly bimodal since there aren't many loans issued with interest rates around 7%) with a right-skew. The median interest rate is `r loans_med` with IQR `r loans_iqr`.
